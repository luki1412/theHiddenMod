...
public void OnPluginStart() 
{
	...
	g_hCV_hidden_ghosts = CreateConVar("sm_thehidden_ghosts", "0", "Sets whether red players should turn into ghosts after death.", FCVAR_NONE, true, 0.0, true, 1.0);
...
public void arena_round_start(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	{   
		for(int client = 1; client <= MaxClients; client++) 
		{
			...
			{
				TF2_AddCondition(client, TFCond_HalloweenInHell, 2147483647.0, 0);
			}
		}
	}
}
...
//set pref
public bool SetClientHiddenPreference(int client, int preference)
{
	char steamId[32];
...
//get pref
public bool GetClientHiddenPreference(int client)
{
	char steamId[32];
	GetClientAuthId(client, AuthId_Steam2, steamId, sizeof(steamId));
	char queryString[100];
...
void GiveHiddenPowers(int i) 
{
	...
	CreateNamedItem(i, "tf_weapon_knife", 4,  1, 0);
	CreateNamedItem(i, "tf_weapon_builder", 735, 1, 0);
}
...
public Action ExplodeBomblet(Handle timer, any ent)
{
	if (IsValidEntity(ent))
	{
		...
		if (IsValidEntity(explosion))
		{
			...
			{
				SetEntPropFloat(physexplosion, Prop_Data, "m_flMagnitude", 500.0);
				SetEntPropFloat(physexplosion, Prop_Data, "m_flRadius", 100.0);
